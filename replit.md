...
- `artifacts/api-server/src/lib/payswitch.ts` + `routes/payswitch.ts` — PaySwitch (TheTeller) integration. `initiateCheckout()` POSTs to `test.theteller.net/checkout/initiate` (or live) with HMAC Basic auth, 12-digit `transaction_id`, amount in pesewas (cedis × 100, zero-padded 12 digits). Routes: `GET /me/subscriber-options` (lists owner/center/vendor/org identities the caller may subscribe for), `POST /payments/payswitch/subscriptions` (auth — creates subscription row in `pending_payment` + payment_transactions row in `pending` + returns `checkoutUrl`), `GET /payments/payswitch/callback?txn=…&code=…&status=…` (NO auth, mounted BEFORE `requireKycVerified` — flips subscription to `active` + writes `subscription_payments` row when `code=000`+`status=successful`, otherwise marks both rows failed; idempotent; redirects to `/billing/result`). Env: `PAYSWITCH_MERCHANT_ID`, `PAYSWITCH_API_USER`, `PAYSWITCH_API_KEY` required; `PAYSWITCH_ENV=test|live` (default test); `PUBLIC_BASE_URL` overrides the redirect origin (otherwise derived from `REPLIT_DOMAINS` then `REPLIT_DEV_DOMAIN`).
