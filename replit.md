...
- `artifacts/api-server/src/lib/payswitch.ts` + `routes/payswitch.ts` — PaySwitch (TheTeller) integration. `initiateCheckout()` POSTs to `test.theteller.net/checkout/initiate` (or live) with HMAC Basic auth, 12-digit `transaction_id`, amount in pesewas (cedis × 100, zero-padded 12 digits). Init routes (all auth, all create a `payment_transactions` row in `pending` + return `checkoutUrl`): `POST /payments/payswitch/subscriptions`, `POST /payments/payswitch/service-invoices/:invoiceId`, `POST /payments/payswitch/parts-orders/:orderId/approve-and-pay`, `POST /payments/payswitch/parts-orders/:orderId/center-pay`, `POST /payments/payswitch/rental-bookings/:bookingId`. `GET /me/subscriber-options` still lists the audiences the caller may subscribe for. `GET /payments/payswitch/callback?txn=…&code=…&status=…` (NO auth, mounted BEFORE `requireKycVerified`) dispatches by the txn row's `purpose`: subscription flip; `closeInvoiceAsPaid` for service invoices; `approveProposalAndReserveStock` (or stamp `paid_by_owner/paid_by_center`) for parts orders; inline `markPaid` flip for rental bookings. On every successful sale callback (excluding subscriptions) it also calls `recordCommission*` to log platform revenue **and** `createPayoutForSale` (lib/payouts.ts) which records a `seller_payouts` row and immediately attempts `disburseToSeller` if `PAYSWITCH_DISBURSE_API_USER`/`KEY` are configured. Redirects always land on `/billing/result?status=…&purpose=<kind>&…ids…` with reason on failure. Env: `PAYSWITCH_MERCHANT_ID`, `PAYSWITCH_API_USER`, `PAYSWITCH_API_KEY` required for charging; `PAYSWITCH_DISBURSE_API_USER` / `PAYSWITCH_DISBURSE_API_KEY` optional for Direct Pay (when missing, payouts queue as `pending` for manual settlement); `PAYSWITCH_ENV=test|live` (default test); `PUBLIC_BASE_URL` overrides the redirect origin.
