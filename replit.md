...
- **Parts payment routing** (mechanic proposals only): owner picks one of two payment paths at approval time via dedicated endpoints — `POST /orders/:orderId/approve-and-pay` (owner pays vendor directly; sets `paymentStatus=paid_by_owner`, reserves stock, status → `placed`) or `POST /orders/:orderId/authorize-center-pay` (owner authorizes the service center to settle with the vendor; sets `centerPayAuthorized=true`, reserves stock, status → `placed`, `paymentStatus` stays `unpaid`). The legacy `PATCH /orders/:orderId/status` no longer accepts `proposed → placed` — rejection (`proposed → cancelled`) still flows through it. Center staff settle via `POST /orders/:orderId/center-pay` (requires `centerPayAuthorized && paymentStatus=unpaid`; sets `paymentStatus=paid_by_center`, `paidAt`). At service-invoice creation time (`POST /bookings/:bookingId/invoice`) the server auto-appends every `paid_by_center` order's line items to the invoice as `kind:"part"` entries (description `"Part: <name> (paid by service center)"`) and stamps `invoicedAt` on those orders so the same parts can never be billed twice. Direct-buy orders are stamped `paid_by_owner` at creation. UI surfaces a `PaymentBadge` (`unpaid` / `paid_by_owner` / `paid_by_center`, with an "Awaiting center payment" variant when authorized but still unpaid) on `OrderDetail` and on the parts-orders panel of `BookingDetail`.
