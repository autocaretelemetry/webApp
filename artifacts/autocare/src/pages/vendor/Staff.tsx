...
import { getListVendorStaffQueryKey } from "@/lib/queryKeys";
...
const ALL_PERMS: { key: string; label: string; hint: string }[] = [
  { key: "manage_parts", label: "Catalog", hint: "Add, edit and price parts" },
  { key: "manage_orders", label: "Orders", hint: "Process and fulfil customer orders" },
  { key: "manage_delivery", label: "Delivery team", hint: "Onboard riders and assign couriers" },
  { key: "view_finance", label: "Finance", hint: "See revenue, payouts and invoices" },
...
export default function VendorStaff() {
  ...
  const { data: staff, isLoading } = useListVendorStaff(vendorId, {
    query: {
      enabled: !!vendor,
      queryKey: getListVendorStaffQueryKey(vendorId),
    },
  });

  const create = useCreateVendorStaff();
  const update = useUpdateVendorStaff();
  const remove = useDeleteVendorStaff();
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: getListVendorStaffQueryKey(vendorId) });
  ...
  const deleteStaff = async (id: string, name: string) => {
    if (!vendor) return;
    try {
      await remove.mutateAsync({ vendorId: vendor.id, staffId: id });
      await invalidate();
  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-500">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          title="My staff"
          description={`${staff?.length ?? 0} team members. Managers have full access; staff get only the permissions you tick.`}
        />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="vs-phone">Phone (optional)</Label>
          <Input
            id="vs-phone"
            value={draft.phone}
            placeholder="+233 ..."
